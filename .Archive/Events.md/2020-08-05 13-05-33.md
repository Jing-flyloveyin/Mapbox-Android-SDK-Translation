...
MapView在构建和更改地图时会经历一系列生命周期事件。 Maps SDK提供了许多不同的更改侦听器界面，以告知您何时发生了特定的地图事件。

不是将侦听器添加到MapboxMap对象，而是将每个侦听器添加到MapView对象。 例如`OnDidFinishRenderingMapListener`和`addOnDidFinishLoadingMapListener`。
