...
Maps SDK提供了一个方便的侦听器，用于在用户点击标记时进行捕获。 默认情况下，所有标记都带有“ onMarkerClick”事件侦听器，用于显示和隐藏信息窗口。 您可以覆盖此默认事件侦听器，并使用`setOnMarkerClickListener`方法进行设置。

要显示带有单击标记标题的祝酒消息，请使用setOnMarkerClickListener监听click事件，最后调用Toast.makeText（）。 为了避免同时显示祝酒消息和信息窗口，请在末尾返回true：
