...
Maps SDK中的` MapboxMap` 类有多种更改相机位置的方法。 每个相机移动API都包含在`CameraUpdate` 对象。 应该使用`CameraUpdateFactory`类提供新的摄像机位置信息。 `CameraUpdateFactory` 可以构建几个不同的` CameraUpdate` 对象，包括` newLatLngZoom()`，`zoomBy()`，` newLatLngBounds()` 等。 ` CameraUpdate` 中一个直接传递内置`CameraPosition `的方法是` newCameraPosition()` 。
