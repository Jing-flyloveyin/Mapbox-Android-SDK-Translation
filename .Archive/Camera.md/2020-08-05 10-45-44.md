...
Maps SDK中的` MapboxMap` 类有多种更改相机位置的方法。 每个相机运动API都包含一个“ CameraUpdate”对象。 您应该使用`CameraUpdateFactory`类提供新的摄像机位置信息。 CameraUpdateFactory可以构建几个不同的CameraUpdate对象，包括newLatLngZoom（），zoomBy（），newLatLngBounds（）等。 在“ CameraUpdate”中，一个简单的方法是“ newCameraPosition（）”，这是您传递内置的“ CameraPosition”的方式。
