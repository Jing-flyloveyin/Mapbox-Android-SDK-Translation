...
摄像机还可以在地图区域内居中，例如如何将摄像机限制在某个区域（请参阅下文）。 首先，您需要一个定义的`LatLngBounds`对象，该对象至少包含两个坐标。 然后，您将可以使用可用的“ newLatLngBounds（）”方法来更新相机位置，该方法将使用您的边界框并调整视口，以使指定区域位于视图内。 除了将边界框传递到相机更新工厂之外，您还需要提供一个“ int”值，以定义屏幕边缘与实际边界区域之间的填充。 您还可以选择为盒子的每一侧提供不同的填充值。
